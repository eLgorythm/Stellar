...
/// Cache global untuk sertifikat dan private key agar tidak perlu regenerasi 
/// atau I/O disk berulang kali setelah inisialisasi pertama.
static CERT_CACHE: OnceCell<(X509, PKey<Private>)> = OnceCell::const_new();
...
async fn setup_tls(port: u16, _storage_dir: &str) -> Result<tokio_boring::SslStream<TcpStream>> {
    let (cert, pkey) = get_persistent_cert(_storage_dir).await?;
    ...
    let mut config = connector.build().configure().context("Failed to configure SSL")?;
...
where S: AsyncReadExt + AsyncWriteExt + Unpin {
    ...
    // 2. Siapkan PeerInfo
    // Operasi encode_rsa_adb_format cukup berat, kita pindahkan ke worker thread.
    let (cert, _) = get_persistent_cert(storage_dir).await?;
    let peer_info = task::spawn_blocking(move || -> Result<Vec<u8>> {
        let rsa = cert.public_key()?.rsa().context("Failed to get RSA from cert")?;
        
        // Operasi matematika berat (Montgomery Reduction) terjadi di sini
        let adb_pub_key = encode_rsa_adb_format(&rsa)
            .context("Failed to encode RSA to ADB format")?; 
...
/// Fungsi untuk "memanaskan" sertifikat saat aplikasi startup.
/// Ini akan memastikan CERT_CACHE terisi sebelum user menekan tombol pairing.
pub async fn pre_warm_cert(storage_dir: String) -> Result<()> {
    let _ = get_persistent_cert(&storage_dir).await?;
    info!("[PAIR] Pre-warming certificate completed.");
    Ok(())
}

/// Mengambil sertifikat dari penyimpanan internal atau membuatnya jika belum ada.
pub(crate) async fn get_persistent_cert(storage_dir: &str) -> Result<(X509, PKey<Private>)> {
    // Cek cache memory terlebih dahulu (sangat cepat)
    if let Some(cached) = CERT_CACHE.get() {
        return Ok(cached.clone());
    }

    let path = std::path::Path::new(storage_dir).join("adb_cert.pem");
    
    let (cert, pkey) = if path.exists() {
        // Baca dari disk jika ada
        ...
        let (cert, pkey) = generate_self_signed_cert().await?;
        let mut pem = cert.to_pem()?;
        pem.extend_from_slice(&pkey.private_key_to_pem_pkcs8()?);
        
        if !fs::try_exists(storage_dir).await.unwrap_or(false) {
            fs::create_dir_all(storage_dir).await.context("Failed to create storage directory")?;
        }
        fs::write(&path, pem).await.context("Failed to write permanent certificate file")?;
        (cert, pkey)
    };

    // Simpan ke cache sebelum dikembalikan
    let _ = CERT_CACHE.set((cert.clone(), pkey.clone()));
    Ok((cert, pkey))
}

async fn generate_self_signed_cert() -> Result<(X509, PKey<Private>)> {
    // Pembuatan kunci RSA 2048 adalah operasi yang sangat memblokir CPU.
    // Kita jalankan seluruh proses pembuatan sertifikat di thread pool.
    task::spawn_blocking(|| -> Result<(X509, PKey<Private>)> {
        let rsa = Rsa::generate(2048).context("Failed to generate RSA key")?;
        let pkey = PKey::from_rsa(rsa.clone()).context("Failed to create PKey")?;
        ...
        // Signing adalah bagian yang berat secara matematis
        builder.sign(&pkey, MessageDigest::sha256())?;

        Ok((builder.build(), pkey))
    }).await.context("RSA Certificate generation task panicked")?
}
